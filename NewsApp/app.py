...
fname = 'doc2vec_model2'
model_dbow = Doc2Vec.load(fname)

loaded_model = load('clf_neural_new2.joblib') 
...
def upload():
    if request.method == 'POST':
        ...
        print(preprocessed)
        word_tokens = word_tokenize(preprocessed) 
        vector = model_dbow.infer_vector(word_tokens)
        vector = vector.reshape(1, -1)
        model_prediction = loaded_model.predict(vector)
        
        result = class_list[model_prediction[0]]
        print(result) 
        return result
    return None
